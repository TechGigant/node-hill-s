...
export default async function checkAuth(socket: ClientSocket, reader: SmartBuffer): Promise<AuthenticationResponse> {
    ...
    if (USER.token.length > 36 || !UID_REGEX.test(USER.token)) return "Token format is incorrect."

    try {
        const data = (await phin({ url: AUTHENTICATION_API(USER.token, Game.serverSettings.hostKey) })).body
        if (!data.error) {
            return {
